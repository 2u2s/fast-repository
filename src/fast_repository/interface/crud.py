...
class CRUDRepositoryInterface(ABC, Generic[EntityT]):
    ...
    def stmt_override(
        ...
        Used as a plain ``with`` block (not ``async with``); it performs no I/O. Outside
        the block the repository reverts to its previous statement. Pass a ``Select`` to
        replace the base statement outright, or a callable that receives the current
        effective statement and returns a modified one (e.g.
        ``lambda s: s.where(User.active)``).

        The override is scoped to this instance and the current execution context, so
        sharing a repository across coroutines is safe; nested blocks compose.
